#!/usr/bin/env python3
"""Train and evaluate models at different dataset sizes (100, 500, 1000 articles).

Usage:
  python train_ablation.py --size 100
  python train_ablation.py --size 500
  python train_ablation.py --size 1000
"""
...
GRAD_ACCUM = 16
LEARNING_RATE = 2e-4
WARMUP_STEPS = 20  # Scale down for small datasets
LR_SCHEDULER = "cosine"
...
def train_model(train_data: list[dict], output_dir: Path, checkpoint_dir: Path, log_file: Path):
    ...
    save_steps = max(total_steps // 3, 10)  # Save ~3 checkpoints
...
def main():
    ...
    parser.add_argument("--size", type=int, required=True, help="Number of articles to train on")
    ...
    print(f"\n{'='*72}")
    print(f"  ABLATION RUN: {size} articles")
    print(f"  Started: {time.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"{'='*72}\n")

    # Paths
    output_dir = BASE_DIR / "output" / f"ablation_{tag}"
    checkpoint_dir = BASE_DIR / "checkpoints" / f"ablation_{tag}"
    log_file = BASE_DIR / f"training_ablation_{tag}.log"
    eval_output = BASE_DIR / "data" / f"eval_results_ablation_{tag}.json"
    ...
    subset_path = BASE_DIR / "data" / f"train_ablation_{tag}.json"
    subset_path.write_text(json.dumps(train_subset))
    print(f"Saved training subset to {subset_path}")

    # Train
    print(f"\n--- Training [{time.strftime('%H:%M:%S')}] ---")
    train_stats = train_model(train_subset, output_dir, checkpoint_dir, log_file)
    ...
    print(f"\n{'='*72}")
    print(f"  RESULTS: {size} articles ({len(train_subset)} training examples)")
    print(f"{'='*72}")
