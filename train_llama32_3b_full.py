#!/usr/bin/env python3
"""Train Llama-3.2-3B on ALL 4,726 articles using QLoRA 4-bit.

Same hyperparameters as baseline Qwen3-8B full-data run.
Designed for RTX 5070 Ti (16GB VRAM) — Llama-3.2-3B uses ~10GB with QLoRA.

Usage:
  mkdir -p output/llama32_3b_full && python -u train_llama32_3b_full.py 2>&1 | tee output/llama32_3b_full/train.log
"""
...
def train_model(train_data: list[dict], output_dir: Path, checkpoint_dir: Path):
    """Train a LoRA adapter on the given data using QLoRA 4-bit."""
    ...
    # Llama uses <|start_header_id|> chat template
...
def main():
    print(f"\n{'='*72}")
    print(f"  LLAMA-3.2-3B FULL DATA (QLoRA 4-bit, 4726 articles)")
    ...
    print(f"\n{'='*72}")
    print(f"  RESULTS: {MODEL_NAME} — {len(train_data)} articles (full dataset)")
    print(f"{'='*72}")
