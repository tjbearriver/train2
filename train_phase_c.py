#!/usr/bin/env python3
"""Phase C: LoRA rank sweep for Qwen3.5-9B on 1000 articles using bf16 LoRA.

Sweeps LoRA rank (r) while keeping all other hyperparameters identical to baseline.
Uses the same 1000-article subsample (seed=42) as the baseline run.

Usage:
  mkdir -p output/phase_c_r16  && python -u train_phase_c.py --rank 16  --tag phase_c_r16  2>&1 | tee output/phase_c_r16/train.log
  mkdir -p output/phase_c_r32  && python -u train_phase_c.py --rank 32  --tag phase_c_r32  2>&1 | tee output/phase_c_r32/train.log
  mkdir -p output/phase_c_r128 && python -u train_phase_c.py --rank 128 --tag phase_c_r128 2>&1 | tee output/phase_c_r128/train.log
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Phase C: LoRA rank sweep training")
    parser.add_argument("--rank", type=int, required=True, help="LoRA rank (e.g. 16, 32, 128)")
    parser.add_argument("--tag", type=str, required=True, help="Run tag for output dir (e.g. phase_c_r16)")
...
def main():
    ...
    print(f"\n{'='*72}")
    print(f"  PHASE C: RANK SWEEP — Qwen3.5-9B (bf16 LoRA, {NUM_ARTICLES} articles)")
    ...
    print(f"\n{'='*72}")
    print(f"  PHASE C RESULTS: r={lora_r} — {len(train_data)} articles")
    print(f"{'='*72}")
